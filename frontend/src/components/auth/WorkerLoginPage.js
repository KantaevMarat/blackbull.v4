// frontend/src/components/pages/WorkerLoginPage.jsx
...
const WorkerLoginPage = () => {
  ...
  const handleSendCode = async () => {
    console.log('WorkerLoginPage: Нажата кнопка "Отправить код"');
    ...
    if (!phoneRegex.test(phoneNumber)) {
      ...
      console.log('Ошибка: Некорректный формат номера телефона');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/send-otp`, {
        ...
        body: JSON.stringify({ phoneNumber, role: 'worker' }),
      });
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      console.log('WorkerLoginPage: Отправка запроса на проверку OTP...');
      const response = await fetch(`${API_URL}/verify-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phoneNumber, otp: verificationCode, role: 'worker' }),
      });

      const result = await response.json();
      console.log('WorkerLoginPage: Ответ от сервера при проверке OTP:', result);
      ...
      if (response.ok && result.token) {
        ...
        navigate('/'); // Перенаправление на панель рабочего
      } else {
        message.error(result.message || 'Ошибка верификации');
        console.log('WorkerLoginPage: Ошибка верификации:', result.message || 'Неизвестная ошибка');
      }
    } catch (error) {
      message.error('Ошибка при подтверждении кода: ' + error.message);
      console.log('WorkerLoginPage: Ошибка при верификации:', error);
    }
  };

  const handleSwitchToAdminLogin = () => {
    console.log('WorkerLoginPage: Переключение на страницу входа для администратора');
    navigate('/admin-login');
  };

  return (
    <div className="login-container">
      <Title level={2} style={{ textAlign: 'center' }}>Авторизация сотрудника</Title>
      {!isCodeSent ? (
      ...
      <Button type="link" onClick={handleSwitchToAdminLogin} className="switch-button">
